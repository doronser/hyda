...
# MNIST-M taken from https://github.com/liyxi/mnist-m
import os
import warnings
...
DOMAIN_ENUM = {domain: i for i, domain in enumerate(DOMAINS)}


class MNIST(tvd.MNIST):
    def __getitem__(self, item):
        img, target = super().__getitem__(item)
        return img, target, DOMAIN_ENUM['MNIST']


class SVHN(tvd.SVHN):
    def __getitem__(self, item):
        img, target = super().__getitem__(item)
        return img, target, DOMAIN_ENUM['SVHN']
...
class USPS(tvd.USPS):
    def __getitem__(self, item):
        img, target = super().__getitem__(item)
        return img, target, DOMAIN_ENUM['USPS']


class MNISTM(tvd.VisionDataset):
    """MNIST-M Dataset.
    """
    ...
    def __getitem__(self, index):
        ...
        return img, target, DOMAIN_ENUM['MNIST-M']
